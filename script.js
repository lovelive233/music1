...
(function antiDevTools() {
    const lock=()=>{
        ...
        location.href='about:blank';history.replaceState(null,null,'about:blank');
    };
    // 拦截调试快捷键
    document.addEventListener('keydown',e=>{
        const kc=e.keyCode;
        if(kc==123||kc==85||(e.ctrlKey&&e.shiftKey&&(kc==73||kc==67))){
            e.preventDefault();e.stopPropagation();lock();
        }
    },true);
    // 检测调试器打开
    const check=()=>{
        if(window.outerWidth-innerWidth>200||window.outerHeight-innerHeight>200) lock();
    };
    setInterval(()=>{try{debugger;}catch(e){lock();}},100);
